...
public class QuadtreeSpeedTest {
	@Test
	void test() throws IOException {
		...
		System.out.println("test the quadtree implementation for speed with 100 random coordinate pairs");
		...
		for (int i = 0; i < 100; i++) {
			...
			mapGraph.nearestNeighborTree(rand_latitude, rand_longitude);
			finalTime = (System.currentTimeMillis() - startTime);
			sumTime = sumTime + finalTime;
		}
		final double meanTime = sumTime / 100;
		System.out.println("The mean time the quadtree needs is " + meanTime);
		
		assertEquals(true, meanTime <= 10);
